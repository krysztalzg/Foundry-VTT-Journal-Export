...
def run_export(pathToWorldDirectory):
    ...
    create_index_file(directories, entries)
...
def create_journal_folders(directories, entries, pathToWorldDirectory):
    print(f'World name: {pathToWorldDirectory.split(os.path.sep)[-1]}')
    ...
    for path in [d.path for d in directories if d.id in [e.directory for e in entries]]:
        try:
...
def create_index_file(directories, entries):
    pass
